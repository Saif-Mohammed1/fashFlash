import APIFeatures from "@/component/util/APIFeatures";
...
export const updateOne = async (req, Model) => {
  try {
    const data = await req.json();

    if (data.password || data.confirmPassword) {
      throw new AppError("This route isn't available ", 400);
    }

    const doc = await Model.findByIdAndUpdate(req.id, data, {
      new: true,
      runValidators: true,
    });
